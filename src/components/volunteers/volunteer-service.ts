...
export async function fetchVolunteers(filters: VolunteerFilters = {}): Promise<VolunteersResponse> {
  const searchParams = new URLSearchParams();
  
  if (filters.search) searchParams.append('search', filters.search);
  if (filters.type) searchParams.append('type', filters.type);
  if (filters.alumni !== undefined) searchParams.append('alumni', String(filters.alumni));
  ...
  const response = await fetch(`/api/staff/volunteers?${searchParams.toString()}`, {
    cache: 'no-store',
    next: { revalidate: 0 }
  });
